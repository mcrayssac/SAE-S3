...
const getClub = async (nomClub, callback) => {
    ...
    await pool.query(queries.getAllClubs, (async (error, results) => {
        if (error) {
            ...
            getClubs = results.rows;
            for (let i = 0; i < getClubs.length; i += 1) {
                club = getClubs[i].nom_prestataire.toLowerCase();
                if (nomClub[0] === club[0]) {
                    for (c = 1, n = 1; c > 0 && c < club.length; c++, n++) {
                        if (club[c] === " ") c += 1;
                        if (club[c] !== nomClub[n]) c = -1;
                    }
                    if (c > 0) {
                        nomClub = getClubs[i].nom_prestataire;
                        i = getClubs.length;
                    }
                }
            }
    ...
    const getAllPrestataire = async (nomPresta, callback) => {
        ...
        await pool.query(mapQueries.getAllPrestataires, (async (error, results) => {
            if (error) {
                ...
                getPrestas = results.rows;
                for (let i = 0; i < getPrestas.length; i += 1) {
                    club = getPrestas[i].nom_prestataire.toLowerCase();
                    if (nomPresta[0] === club[0]) {
                        for (c = 1, n = 1; c > 0 && c < club.length; c++, n++) {
                            if (club[c] === " ") c += 1;
                            if (club[c] !== nomPresta[n]) c = -1;
                        }
                        if (c > 0) {
                            nomPresta = getPrestas[i].nom_prestataire;
                            i = getPrestas.length;
                        }
                    }
                }
                await pool.query(mapQueries.getPrestas, [nomPresta], ((error, result)=>{
                    if (error) {
                        console.log(error);
                        return callback([]);
                    }
                    else
                        getPresta = result.rows[0];
                    return callback(null, getPresta);
                }));
            }
        }));
