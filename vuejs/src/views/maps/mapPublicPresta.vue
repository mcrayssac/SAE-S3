        ...
        <b-col class="mt-3 svg-col" cols="12" lg="9">
          <div class="map_image" id="map_image">
            ...
            <svg
              ...
              <!-- =============================================================COURSES COMPET============================================================= -->
              <g id="courses" ref="courses">
                <marker
                    markerWidth="5.818182"
                    markerHeight="8"
                    refX="-1"
                    refY="0"
                    orient="0"
                  ...
                  <path
                      style="fill:#ff0000;stop-color:#1c1c1c"
                      d="m 2,-9 c 0,1.1045695 -0.8954305,2 -2,2 -1.1045695,0 -2,-0.8954305 -2,-2 0,-1.104569 0.8954305,-2 2,-2 1.1045695,0 2,0.895431 2,2 z m 2,0 c 0,3 -3,6 -4,7 -1,-1 -4,-4 -4,-7 0,-2.209139 1.790861,-4 4,-4 2.209139,0 4,1.790861 4,4 z"/>
                </marker>
                <a v-for="(course,index) in tabCoursesMap" :key="index" :ref="course.idCourse">
                <path
                    class="course-is-desactive"
                    style="paint-order:stroke fill markers;marker-start:url(#pin);"
                    :id="course.idCourse"
                    :d="course.trace"
                    />
                </a>
        ...
        <b-row class="py-3 mt-3 mx-0 map_list">
          ...
          <b-row class="m-0" align-h="center" align-v="center">
            ...
            <b-col class="mb-3" cols="auto">
              ...
              <br><br>
              <b-form-select v-model="courseSelected" :options="tabCourses" @change="filterCourse()"></b-form-select>
            </b-col>

            <b-col class="mb-3" cols="auto">
              <span class="title-filtre-besoin">Caractéristiques</span>
            </b-col>
          </b-row>

          <b-row class="my-1 ms-1" v-for="(caracteristique, index) in tabCaracteristiques" :key="index">
            <b-col>
              <label class="map-check-label" :for="caracteristique">
                <input type="checkbox" :id="caracteristique" :name="caracteristique" @change="filtreStands(index)">
            ...
            <template #default="{ hide }">
              ...
              <b-row class="mb-3 m-0" align-h="center" align-v="center">
                <b-col cols="auto">
                  ...
                  <b-row align-h="center" align-v="center"
                         v-for="(presta, index) in tabStand" :key="index" :ref="presta.nom_prestataire"
...
export default {
  ...
  methods:{
    ...
    getRotation (index) {
      return 'rotate(' + this.tabStand[index].rotation + ',' + this.tabStand[index].coordonne_x + ', ' + this.tabStand[index].coordonne_y +')'
    },
    ...
    filterCourse(){
      //pour reset
      for(let i = 0; i < this.tabCoursesMap.length; i++) {
        this.$refs[this.tabCoursesMap[i].idCourse][0].getElementsByTagName('path')[0].classList.add('course-is-desactive');
        this.$refs[this.tabCoursesMap[i].idCourse][0].getElementsByTagName('path')[0].classList.remove('course-is-active')
      }
      let tabTemp = this.tabCourses.findIndex(course =>{
        return course==this.courseSelected
      })
      this.$refs[tabTemp+1][0].getElementsByTagName('path')[0].classList.remove('course-is-desactive');
      this.$refs[tabTemp+1][0].getElementsByTagName('path')[0].classList.add('course-is-active');
    },
  ...
  async created() {
    ...
    //liste les types de presta
    await axios.get(`http://localhost:3000/map/type`)
        .then(result => {
          // this.tabCaracteristiques=result.data.data
          result.data.data.forEach(type => {
            this.tabCaracteristiques.push(Object.values(type)[0])
    ...
    await axios.get(`http://localhost:3000/map/caracteristiques`)
        .then(result => {
          result.data.data.forEach(caracteristique => {
    ...
    await axios.get(`http://localhost:3000/map/courses`)
        .then(result => {
          ...
          result.data.data.forEach(course =>{
            ...
            obG = new Object();
            obG.idCourse=course.id_course;
            obG.trace=course.trace;
            this.tabCoursesMap.push(obG)
          })
        })
        .catch((err) => {
          let message = typeof err.response !== "undefined" ? err.response.data.message : err.message;
...
@import '../../../public/css/maps/mapPublic.css';
</style>