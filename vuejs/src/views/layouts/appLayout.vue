<template>
  <div>
    <b-navbar v-if="userInfos.admin === 'organisateur'" class="fixed-top"
              :style="backgroundNavbarColor.title + backgroundNavbarColor.body" style="padding: 3px 0 3px 0;"
              toggleable="lg">
      ...
      <b-collapse id="nav-collapse" is-nav>
        ...
        <b-navbar-nav>
          ...
          <b-nav-item href="/courses"><span class="text-light"><b-icon-trophy-fill></b-icon-trophy-fill> Courses</span>
          </b-nav-item>

          <b-nav-item href="/scene"><span class="text-light"><b-icon-megaphone></b-icon-megaphone> Scène</span>
          </b-nav-item>
        </b-navbar-nav>
        ...
        <b-navbar-nav class="ms-auto me-3">
          ...
          <b-nav-item-dropdown class="nav-item" v-else right toggle-class="text-white">
            <template #button-content><span style="color: #fffb00"><b-icon-person-fill></b-icon-person-fill>Organisateur</span>
            </template>
          ...
          <b-nav-item href="/association" class="removePadding">
            <b-img height="30" width="auto"
                   src="https://upload.wikimedia.org/wikipedia/fr/thumb/1/16/Logo_APF_France_Handicap_2018.svg/langfr-195px-Logo_APF_France_Handicap_2018.svg.png"></b-img>
          </b-nav-item>
    ...
    <b-navbar v-else-if="userInfos.admin === 'prestataire'" class="fixed-top"
              :style="backgroundNavbarColor.title + backgroundNavbarColor.body" style="padding: 3px 0 3px 0;"
              toggleable="lg">
      <b-navbar-brand href="http://localhost:8080/" class="ms-3">
        <img src="https://cdn.discordapp.com/attachments/1019997748344406146/1027862507618058292/logo_3_1.png"
      ...
      <b-collapse id="nav-collapse" is-nav>
        ...
        <b-navbar-nav v-if="userInfos.etat !== null && userInfos.etat !== undefined && userInfos.etat === false">
          <b-nav-item href="/map" disabled><span class="text-light"><b-icon-geo-alt-fill></b-icon-geo-alt-fill> Map</span>
          </b-nav-item>

          <b-nav-item href="/statistiques" disabled><span class="text-light"><b-icon-graph-up></b-icon-graph-up> Statistiques</span>
          </b-nav-item>

          <b-nav-item href="#" disabled><span class="text-light"><b-icon-list-ul></b-icon-list-ul> Initiations</span>
          </b-nav-item>

          <b-nav-item href="/scene" disabled><span class="text-light"><b-icon-journal-check></b-icon-journal-check> Scène</span>
          </b-nav-item>

          <b-nav-item :href="'/prestataires/'+userInfos.name.toLowerCase().trim().replace(/ /g,'')"><span
              class="text-light"><b-icon-person-lines-fill></b-icon-person-lines-fill> Ma Page</span></b-nav-item>

          <b-nav-item href="/etatInscription"><span class="text-light"><b-icon-person-lines-fill></b-icon-person-lines-fill> Mon etat</span>
          </b-nav-item>
        </b-navbar-nav>

        <b-navbar-nav v-else>
          <b-nav-item href="/map"><span class="text-light"><b-icon-geo-alt-fill></b-icon-geo-alt-fill> Map</span>
          </b-nav-item>

          <b-nav-item href="/statistiques"><span class="text-light"><b-icon-graph-up></b-icon-graph-up> Statistiques</span>
          </b-nav-item>

          <b-nav-item href="#"><span class="text-light"><b-icon-list-ul></b-icon-list-ul> Initiations</span>
          </b-nav-item>

          <b-nav-item href="/scene"><span class="text-light"><b-icon-journal-check></b-icon-journal-check> Scène</span>
          </b-nav-item>
    ...
    <b-navbar v-else class="fixed-top" :style="backgroundNavbarColor.title + backgroundNavbarColor.body"
              style="padding: 3px 0 3px 0;" toggleable="lg">
      <b-navbar-brand href="http://localhost:8080/" class="ms-3">
        <img src="https://cdn.discordapp.com/attachments/1019997748344406146/1027862507618058292/logo_3_1.png"
             alt="IUT LOGO" width="45" height="40" class="d-inline-block rounded align-text-top">
      </b-navbar-brand>
      ...
      <b-collapse id="nav-collapse" is-nav>
        ...
        <b-navbar-nav class="ms-auto me-3">
          ...
          <b-nav-item-dropdown class="nav-item" v-else right toggle-class="text-white">
            <template #button-content>
              <b-icon-person-fill></b-icon-person-fill>
              Bonjour {{ userInfos.name }}
            </template>
            <b-dropdown-item href="#">Planning</b-dropdown-item>
            <b-dropdown-item href="#">Mes activités</b-dropdown-item>
            <b-dropdown-item @click="logout()">Déconnexion</b-dropdown-item>
            <b-dropdown-item @click="showLoginErrorModal('login-after-error-modal')">Supprimer le compte
            </b-dropdown-item>
          </b-nav-item-dropdown>
    ...
    <b-modal body-text-variant="danger" ref="login-error-modal" title="Lakeside Sports Festival" hide-header-close>
      <b-col class="my-3">
        <b-row class="my-1" align-h="center">
          <b-col cols="auto">
            <span
                style="font-size: x-large; font-family: 'Montserrat', sans-serif;">Email ou mot de passe erroné !</span>
          </b-col>
        </b-row>
        <b-row class="my-1" align-h="center">
          <b-col cols="auto">
    ...
    <b-modal body-text-variant="danger" ref="login-after-error-modal" title="Lakeside Sports Festival"
             hide-header-close>
      <b-col class="my-3">
        <b-row class="my-1" align-h="center">
          <b-col cols="auto">
            <span
                style="font-size: large; font-family: 'Montserrat', sans-serif;">Vous aller supprimer votre compte</span>
          </b-col>
        </b-row>
        <b-row class="my-1" align-h="center">
          <b-col cols="auto">
...
export default {
  ...
  data: () => ({
    ...
    allLanguage: [{
      "title": "English",
      "flag": "https://upload.wikimedia.org/wikipedia/commons/thumb/8/83/Flag_of_the_United_Kingdom_%283-5%29.svg/langfr-338px-Flag_of_the_United_Kingdom_%283-5%29.svg.png"
    }],
      ...
      this.$store.dispatch('login', {
        ...
        setTimeout(() => {
          ...
          if (!self.userInfos.admin || self.userInfos.admin === "organisateur") {
            ...
            if (self.userInfos.etat) self.$router.push({
              name: 'prestataires/nomPrestataire',
              params: {nomPrestataire: self.userInfos.name.toLowerCase().trim().replace(/ /g, '')}
            });
